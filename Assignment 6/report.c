...
{
  sem_wait(&(bank->reportLock)); // vaatomurebt metods
...
{
  sem_wait(&(bank->checker));
  bank->numberWorkersHasToFinish--;
  if (bank->numberWorkersHasToFinish != 0)
  {
    sem_post(&(bank->checker));
    sem_wait(&(bank->startNextDay));
    sem_post(&(bank->quickLock));
    return 0;
  }
  else
  {
    ...
    if (rpt->numReports >= MAX_NUM_REPORTS)
    {
      for(int i = 0; i < numWorkers - 1; i++){
        sem_post(&(bank->startNextDay));
      }

      for(int i = 0; i < numWorkers - 1; i++){
        sem_wait(&(bank->quickLock));
      }
