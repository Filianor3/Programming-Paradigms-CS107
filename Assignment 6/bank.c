          ...
          int numWorkers)
{
  ...
  sem_init(&(bank->quickLock), 0, 0);
  sem_init(&(bank->branchFix), 0, 1);

  // if we call bank init, it means the work just started to we assign the amount of numworkers to
  // the amount of workers that has to finish the day
  bank->numberWorkersHasToFinish = numWorkers; 

  // nobody yet wants to go to second day so we make it 0
  sem_init(&(bank->startNextDay), 0, 0); 

  // anyone can start checking so the arguement is 1
