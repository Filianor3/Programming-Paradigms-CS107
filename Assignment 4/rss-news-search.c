...
static void BuildIndices(const char *feedsFileName, hashset *restOfTheWords, hashset *stopWords);
static void ProcessFeed(const char *remoteDocumentName, hashset *restOfTheWords, hashset *stopWords);
static void PullAllNewsItems(FILE *dataStream, hashset *restOfTheWords, hashset *stopWords);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *restOfTheWords, hashset *stopWords);
...
static void ParseArticle(const char *articleTitle,
                         ...
                         const char *articleURL, hashset *restOfTheWords,
 	hashset *stopWords);
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
                        const char *unused, const char *articleURL, hashset *restOfTheWords,
 	hashset *stopWords);
static void QueryIndices(hashset *stopWords, hashset *restOfTheWords);
static void ProcessResponse(const char *word, hashset *stopWords, hashset *restOfTheWords);
static bool WordIsWellFormed(const char *word);
static void buildStopHashset(hashset *stopWords);
static void buildNormalWordHashset(hashset *normalWords);
...
static int vectorComparator(const void* s1, const void* s2);
static void secStructFree(void *s);
static void entryFree(void *s);
static int entryHash(const void *s, int numBuckets);
static int entryCompare(const void* s1, const void* s2);
static void insertWordInHashset(char* word, hashset* restOfTheWords, hashset* stopWords,
				const char *articleTitle, const char *articleURL);
static void disposeEverything(hashset *stopWords, hashset *restOfTheWords);		
/**
 * Function: main
 * --------------
...
static const char *const kTextDelimiters =
    " \t\n\r\b!@$%^*()_+={[}]|\\'\":;/?.>,<~`";
static const int numBuckets = 1669;
...
int main(int argc, char **argv) {
	hashset stopWords, restOfTheWords;
	buildStopHashset(&stopWords);	
	HashSetNew(&restOfTheWords, sizeof(entry), numBuckets, entryHash, entryCompare, entryFree);
	setbuf(stdout, NULL);
  	curl_global_init(CURL_GLOBAL_DEFAULT);
  	Welcome(kWelcomeTextFile);
	BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1], &restOfTheWords, &stopWords);
	QueryIndices(&stopWords, &restOfTheWords);
	curl_global_cleanup();
	disposeEverything(&stopWords, &restOfTheWords);
	return 0;
}

static void disposeEverything(hashset *stopWords, hashset *restOfTheWords){
	HashSetDispose(stopWords);
	HashSetDispose(restOfTheWords);
}

static void buildStopHashset(hashset *stopWords){
	HashSetNew(stopWords, sizeof(char*), numBuckets, StringHash, StringCompare, StringFree);
	streamtokenizer st;
	FILE* stopWordFile = fopen("data/stop-words.txt", "r");
	STNew(&st, stopWordFile, kTextDelimiters, true);
	char buffer[256];
...
static void BuildIndices(const char *feedsFileName,
 	hashset *restOfTheWords, hashset *stopWords) {
  FILE *infile;
  streamtokenizer st;
  ...
  while (STSkipUntil(&st, ":") !=
    ...
    ProcessFeed(remoteFileName, restOfTheWords, stopWords);
  }
...
static void ProcessFeedFromFile(char *fileName, hashset *restOfTheWords,
 	hashset *stopWords) {
  FILE *infile;
  streamtokenizer st;
  ...
  ScanArticle(&st, (const char *)fileName, articleDescription,
              (const char *)fileName, restOfTheWords, stopWords);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew
                  // doesn't open one..
  fclose(infile);
}
...
static void ProcessFeed(const char *remoteDocumentName,
	hashset *restOfTheWords,
 	hashset *stopWords) {
	
  if (!strncmp(kFilePrefix, remoteDocumentName, strlen(kFilePrefix))) {
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(kFilePrefix), restOfTheWords,
 	stopWords);
    return;
  }
  FILE *tmpFeed = FetchURL(remoteDocumentName, "tmp_feed");
  PullAllNewsItems(tmpFeed, restOfTheWords, stopWords);
  fclose(tmpFeed);
}
...
static void PullAllNewsItems(FILE *dataStream, hashset *restOfTheWords,
 	hashset *stopWords) {
  streamtokenizer st;
  ...
  while (GetNextItemTag(
    ...
    ProcessSingleNewsItem(&st, restOfTheWords, stopWords);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *restOfTheWords,
 	hashset *stopWords) {
  char htmlTag[1024];
  ...
  ParseArticle(articleTitle, articleDescription, articleURL, restOfTheWords, stopWords);
}
...
static void ParseArticle(const char *articleTitle,
  ...
  ScanArticle(&st, articleTitle, articleDescription, articleURL, restOfTheWords, stopWords);
  STDispose(&st);
  fclose(tmpDoc);
}
...
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
	...
	while (STNextToken(st, word, sizeof(word))) {
      ...
      //printf("11111111111111111111111111111111\n");
			insertWordInHashset(word, restOfTheWords, stopWords, articleTitle, articleURL);
...
static void insertWordInHashset(char* word, hashset* restOfTheWords, hashset* stopWords,
	...
	entry currEntry;
	currEntry.ch = strdup(word);
	entry* temp = HashSetLookup(restOfTheWords, &currEntry);
	if(temp != NULL){
		free(currEntry.ch);
		//free(currEntry);
		vector* curVec = &((temp)->urlFrq);
    char* art = strdup(articleURL);
    //printf("1111111111111111111111111\n");
    //printf("%d %s %s %s %s \n", VectorLength(curVec), word, temp->ch, art, 
    // ((urlAndFreq*)VectorNth(curVec, VectorLength(curVec) - 1))->url);
    //printf("0000000000000000000000\n");
		if(strcasecmp(((urlAndFreq*)VectorNth(curVec, VectorLength(curVec) - 1))->url, art) == 0){
      ((urlAndFreq*)VectorNth(curVec, VectorLength(curVec) - 1))->freq++;
		}else{
			urlAndFreq newElem;
			newElem.freq = 1;
			newElem.title = strdup(articleTitle);
			newElem.url = strdup(articleURL);
			VectorAppend(curVec, &newElem);
		}
    free(art);
	}else{
		VectorNew(&(currEntry.urlFrq), sizeof(urlAndFreq), secStructFree, 1);
		urlAndFreq newElem;
		newElem.freq = 1;
		newElem.title = strdup(articleTitle);
		newElem.url = strdup(articleURL);
		VectorAppend(&currEntry.urlFrq, &newElem);
		HashSetEnter(restOfTheWords, &currEntry);
	}
}
...
static void QueryIndices(hashset *stopWords, hashset *restOfTheWords) {
  ...
  while (true) {
    ...
    if (strcasecmp(response, "") == 0)
      break;
    ProcessResponse(response, stopWords, restOfTheWords);
  }
}
...
static void ProcessResponse(const char *word, hashset *stopWords, hashset *restOfTheWords) {
    ...
    if (WordIsWellFormed(word) ) {
    if(HashSetLookup(stopWords, &newWord) != NULL){
      printf("Too common a word to be taken seriously. Try something more specific.\n");
      free(newWord);
      return;
    }
    //printf("0000000000000\n");
		entry* chosenWord = malloc(sizeof(entry));
		chosenWord->ch = (char*)newWord;
		entry* XD = (entry*)HashSetLookup(restOfTheWords, chosenWord);
    if(XD == NULL){
      ...
      return;
		...
		VectorSort(&XD->urlFrq, vectorComparator);
    //[search term occurs 30 times]
    int iterSize = 10;
    if(VectorLength(&XD->urlFrq) < 10) iterSize = VectorLength(&XD->urlFrq);
    for(int i = 0; i < iterSize; i++){
      urlAndFreq* curr = VectorNth(&XD->urlFrq, i);
      printf("%d.) \"%s\" [search term occurs %d %s]\n\"%s\"\n", i + 1, curr->title, curr->freq, curr->freq == 1 ? "time" : "times", curr->url);
    }
		free(chosenWord);
  } else {
    printf(
...
static int StringHash(const void *s, int numBuckets)  {            
  int i;
  char* strToHash = *((char**)s);
  unsigned long hashcode = 0;
  
  for (i = 0; i < strlen(strToHash); i++)  
    hashcode = hashcode * kHashMultiplier + tolower(strToHash[i]);  
  
  return hashcode % numBuckets;                                
}

static int StringCompare(const void* s1, const void* s2){
	char* strToComp1 = *((char**)s1);
	char* strToComp2 = *((char**)s2);
	return strcasecmp(strToComp1, strToComp2);
}

static void StringFree(void *s){
	char* strToFree = *((char**)s);
	free(strToFree);
}

static int entryHash(const void *s, int numBuckets){            
  int i;
  char* strToHash = ((entry*)s)->ch;
  unsigned long hashcode = 0;
...
static int entryCompare(const void* s1, const void* s2){
	char* strToComp1 = ((entry*)s1)->ch;
	char* strToComp2 = ((entry*)s2)->ch;
	return strcasecmp(strToComp1, strToComp2);
}

static void entryFree(void *s){
	entry* toFree = (entry*)s;
	char* strToFree = toFree->ch;
	vector* vecToFree = &toFree->urlFrq;
	VectorDispose(vecToFree);
	free(strToFree);
	//free(toFree);
}

static void secStructFree(void *s){
	urlAndFreq* toFree = (urlAndFreq*)s;
	char* url = toFree->url;
	char* title = toFree->title;
	//free(toFree);
	free(url);
	free(title);
}
...
static int vectorComparator(const void* s1, const void* s2){
	return ((urlAndFreq*)s2)->freq - ((urlAndFreq*)s1)->freq;
}