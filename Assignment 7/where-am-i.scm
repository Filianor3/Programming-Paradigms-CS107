...
(define (intersection-points my_list)
   ;; yvela wre (A, B, C, D) 
   ;; A-ze gamovidzaxe, gavatane danarcheni sxva yvela wrewiri
   ;; jer B mere moashore B mere C mere moashore C merte D moashore D
   ;; recursive call yvela wris pirveli wevristvis gadavcet wreebi tavisi tavis gareshe
   ;; yovel gamodzaxebaze, vidzaxebt wreebis gadakvetis metods, except wina gamodzaxebaze amorcheul
   ;; wres vagdebt siidan, manam sanam chveni siis zoma ar iqneba 0
   (if (= (length my_list) 0) '() 
    (let ((mainCircle (car my_list)))
     (apply append (append (map 
                   (lambda (secondCircle) (intersect mainCircle secondCircle)) 
                   (cdr my_list))
                   (list (intersection-points (cdr my_list))))
                   )
    )
   )
)

(define (distance-product mrPoint my_list)
  (apply * (map (lambda (mrsPoint) (dist mrPoint mrsPoint)) (remove mrPoint my_list)))
)


(define (rate-points my_list)
  (map (lambda (mrPoint) (list (distance-product mrPoint my_list) mrPoint)) my_list)
)

(define (sort-points my_list)
  (quicksort my_list)
)

(define (clumped-points my_list)
  (if (= (remainder (length my_list) 2) 1)
    (prefix-of-list (map (lambda (mrPoint)
           (cadr mrPoint))
         (sort-points (rate-points my_list))) (/ (- (length my_list) 1) 2))

    (prefix-of-list (map (lambda (mrPoint)
           (cadr mrPoint))
         (sort-points (rate-points my_list))) (/ (length my_list) 2))
  ) 
)

(define (average-point my_list)
  (let ((mrX (/ (apply + (map (lambda (mrPoint) (car mrPoint)) my_list)) (length my_list)))
       (mrY (/ (apply + (map (lambda (mrPoint) (cadr mrPoint)) my_list)) (length my_list)))
       
  )
  (let ((mrsPoint (make-pt mrX mrY)))
  (list (distance-product mrsPoint my_list) mrsPoint)
  ))
)

(define (best-estimate my_list)
  (average-point (clumped-points (intersection-points my_list)))
)

(define (where-am-i my_list_d my_list_s)
  (sort-points (map best-estimate (all-guesses my_list_d my_list_s)))
)
