...
void actuallyGenerate(map<string, Definition> &grammar, vector<string> &generated, string start){
  if(!(start[0] == '<' && start[start.size() - 1] == '>')){
    generated.push_back(start);
  }
  if(!grammar.count(start)) return;
  Production prod = grammar[start].getRandomProduction();
  for(Production::iterator it = prod.begin(); it < prod.end(); it++){
    actuallyGenerate(grammar, generated, *it);
  }
}

void generate(map<string, Definition> &grammar, int version){
  vector<string> generated;
  actuallyGenerate(grammar, generated, "<start>");
